...
const (
	retryDelay      = centigram.SwapRetryDelay
	...
	pmsTimeOut      = centigram.PmsTimeout
	pmsSyncTimeout  = centigram.PmsSyncTimeout
...
func (p *Plugin) registerStateMaschine() {
	...
	// on refusion next packet
	p.automate.RegisterRule(template.PacketRefusion, commandSent, p.automate.HandlePacketAcknowledge, dispatcher.StateAction{})

	// ack for TplMessageLampOn
	p.automate.RegisterRule(template.PacketMessageWaitingStatusOnSwap, commandSent, p.handlePacket, dispatcher.StateAction{})
