...
import (
	...
	visionline "github.com/weareplanet/ifcv5-drivers/visionline/dispatcher"
	...
	"github.com/weareplanet/ifcv5-drivers/visionline/dispatcher"
...
func New(parent *visionline.Dispatcher) *Plugin {
	return &Plugin{driver: parent}

}

const (
	retryDelay      = visionline.RetryDelay
	packetTimeout   = visionline.PacketTimeout
	nextActionDelay = visionline.NextActionDelay
	aliveTimeout    = visionline.AliveTimeout
	maxError        = visionline.MaxError
	maxNetworkError = 2
...
func (p *Plugin) registerStateMaschine() {
	...
	automate.RegisterRule(template.PacketAlive, linkDown, p.handleAlive, dispatcher.StateAction{NextState: linkUp, NextTimeout: nextActionDelay})
	automate.RegisterRule(template.PacketAlive, linkUp, p.handleAlive, dispatcher.StateAction{NextState: linkUp, NextTimeout: nextActionDelay})
}
