...
func (p *Plugin) handleConnection(addr string) {
	...
	go func(addr string) {
		...
		route, err := dispatcher.RegisterPacketRoute(name, addr, template.PacketAlive)
		...
		startState := linkUp // linkDown -> we start with send alive Heartbeat and set linkUp on received answer heartbeat

		if startState == linkUp {
			dispatcher.ChangeLinkState(addr, linkUp) // if we start with linkUp, trigger link state immediately
		} else {
