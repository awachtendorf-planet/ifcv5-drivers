...
func (p *Plugin) preCheck(addr string, job *order.Job) error {
	...
	case order.KeyDelete:
		if p.isSharer(job.Context) && !p.hasReservationNumber(job.Context) {
			return errors.New("key delete without reservation-id is not allowed if sharer flag is set")
		}
	}
...
func (p *Plugin) hasReservationNumber(context interface{}) bool {
	if guest, ok := context.(*record.Guest); ok {
		return (len(guest.Reservation.ReservationID) > 0)
	}
	return false
}
...
func (p *Plugin) handleJob(addr string, encoder int, job *order.Job) {
	...
	go func(addr string, job *order.Job) {
		...
		handleKeyAnswer := func(addr string, in *ifc.LogicalPacket, action *dispatcher.StateAction, job *order.Job) error {
			...
			encoderAddr, _ := driver.GetEncoder(in)
			...
			if encoder == encoderAddr {
				...
				case order.KeyChange:
					if in.Name != template.PacketCardUpdate {
						log.Debug().Msgf("%s addr '%s' ignore incoming packet '%s' because we expect '%s'", name, addr, in.Name, template.PacketCardUpdate)
						break
					}
				...
				case order.KeyRead:
					if in.Name != template.PacketReadKey {
						log.Debug().Msgf("%s addr '%s' ignore incoming packet '%s' because we expect '%s'", name, addr, in.Name, template.PacketReadKey)
						break
					}
				...
				case order.KeyDelete:
					if in.Name != template.PacketCheckout {
		...
		automate.RegisterRule(template.PacketReadKey, answer, handleKeyAnswer, dispatcher.StateAction{})
		automate.RegisterRule(template.PacketCardUpdate, answer, handleKeyAnswer, dispatcher.StateAction{})
		...
		err := automate.StateMaschine(
			...
			template.PacketReadKey,
			template.PacketCardUpdate,
		)
