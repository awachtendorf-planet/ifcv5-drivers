...
func (d *Dispatcher) UnmarshalPacket(packet *ifc.LogicalPacket, p interface{}) error {
	...
	for i := 0; i < reflectType.NumField(); i++ {
		...
		id := strings.Split(packet.Name, " ")
		if len(id) > 1 && len(id[0]) > 1 && len(id[1]) > 1 {
			if mapped, err := d.GetMapping("map.inbound", station, fmt.Sprintf("%c%c%s", id[0][0], id[1][0], key), true); err == nil {
				key = mapped
			}
		}
		...
		if value, exist := d.GetField(packet, key); exist {
			...
			if key == "SO" {
				value = d.GetPMSMapping("Generic Outlet", station, key, value)
			} else {
			...
			case reflect.Struct:
				value := fmt.Sprintf("%s", reflectValue.Field(i))
				err = errors.Errorf("struct are not supported (%s)", value)
...
func (d *Dispatcher) MarshalPacket(p interface{}) (*record.Generic, error) {
	...
	for i := 0; i < reflectValue.NumField(); i++ {
		...
		case reflect.Struct:
			value := fmt.Sprintf("%s", reflectValue.Field(i))
			// guess it is time.Time
			if date, err := time.Parse("2006-01-02 15:04:05 -0700 MST", value); err == nil && !date.IsZero() {
				answer.Set(key, date)
				continue
			}
			err = errors.Errorf("struct are not supported (%s)", value)
