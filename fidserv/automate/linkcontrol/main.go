...
func (p *Plugin) handleConnection(addr string) bool {
	...
	go func(addr string) {
		...
		route, err := dispatcher.RegisterPacketRoute(name, addr,
			template.PacketLinkStart,
			template.PacketLinkAlive,
			template.PacketLinkEnd,
			template.PacketLinkDescription,
			template.PacketLinkRecord,
		)
		if err != nil {
			log.Error().Msgf("%s", err)
			p.state.Remove(addr)
		...
		for {
			...
			// shutdown
			case <-p.kill:
				// did not work here, because parts of the interface already disabled
				// we need a extra trigger, a kind of pre-shutdown trigger
				// action := dispatcher.StateAction{NextTimeout: retryDelay}
				// p.sendLinkEnd(addr, nil, &action, nil)
				return
