...
func (p *Plugin) handleNextAction(addr string, t *ticker.ResetTicker, job *order.Job) {
	...
	case nextAction:
		...
		if job == nil { // should not happend, safety check
			automate.NextAction(name, addr, success, t)
			return
		}

		action.CurrentState = busy // on error fallback to send enquiry
		action.NextState = sendSOF // send SOF and wait for answer afterwards
		...
		if packet, exist := p.workflow.Get(int(job.Action), uint64(job.Task)); exist {
			...
			// should not happend, safety check
	...
	case waitForAnswer:
		...
		p.handleError(addr, job, "key system did not answer")
	...
	case sendSOF:
		action.CurrentState = busy       // on error fallback to send enquiry
		action.NextState = waitForAnswer // wait for answer package
		action.NextTimeout = p.getKeyAnswerTimeout(addr)
		err = p.sendPacket(addr, &action, template.PacketSof, job.Initiator, job.Context)
...
func (p *Plugin) handleKeyAnswer(addr string, packet *ifc.LogicalPacket, action *dispatcher.StateAction, job *order.Job) error {
	...
	case template.PacketCodeCardAnswer:
		...
		errorCode := p.getKeyAsString("Key", packet)
		...
		if errorCode == "P" {
			...
			answer := p.getKeyAsString("Value", packet)
