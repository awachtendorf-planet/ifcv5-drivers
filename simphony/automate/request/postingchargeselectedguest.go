...
func (p *Plugin) handlePostingCharge(addr string, packet *ifc.LogicalPacket, action *dispatcher.StateAction) error {
	...
	guestID := p.getField(packet, "GuestID", true)
	...
	case 1: // posted successfully
		successMessage = setLength(fmt.Sprintf("%s / %s", postingResponse.ResponseText, postingResponse.GuestName), 30)
	case 2:
...
func (p *Plugin) getTotals(data []string, numSalesItemizer int) []record.ChargeItem {

	var totals []record.ChargeItem

	for i := 0; i < numSalesItemizer; i++ {

		subTotal := cast.ToFloat64(data[i])
