...
func (d *Dispatcher) GetAdditionalLRCSize(addr string, name string) int {
	...
	switch name {
	case template.PacketAck, template.PacketNak, template.PacketEnq, template.PacketTermination:
		return 0
	}
...
func (d *Dispatcher) ProcessIncomingLRC(addr string, data []byte, _ int, LRC []byte, _ string) error {
	var err error
	if len(LRC) < 1 {
	...
	if err == nil {
		...
		for _, char := range data {
			...
			case 0x03:
				...
				if calculated^0x40 == LRC[0] {
					return nil
				}

				err = errors.Errorf("crc mismatch, received 0x%x, calculated: PMS1 0x%x, PMS2 0x%x", LRC[0], calculated, calculated^0x40)
