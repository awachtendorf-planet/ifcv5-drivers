...
func (d *Dispatcher) initOverwrite() {
	...
	// handle external parser templates
	d.ConfigPreProcess = d.configPreProcess
}

func (d *Dispatcher) configPreProcess(file string, data *[]byte) bool {
	processed := d.handleTemplate(file, data)
	return processed
}
...
func (d *Dispatcher) PreCheck(job *order.Job) error {
	...
	return errors.Errorf("driver does not support this command (%s)", job.Action.String())
}
