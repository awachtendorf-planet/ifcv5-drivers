...
func (p *Plugin) handleKeyAnswer(addr string, packet *ifc.LogicalPacket, action *dispatcher.StateAction, job *order.Job) error {
	...
	if devAddr, err := dispatcher.NormalizePhysicalAddr(addr); err == nil {
		dispatcher.SetAlive(devAddr)
	}
	...
	case template.PacketCodeCardAnswer:
		...
		keyId := p.getKeyAsString("Data", packet)
		p.handleSuccess(addr, job, keyId)

		action.NextState = success
		action.NextTimeout = nextActionDelay

	case template.PacketError:
		errorCode := p.getKeyAsString("ErrorCode", packet)
		answer := driver.GetAnswerText(errorCode)
