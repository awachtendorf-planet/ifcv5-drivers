...
func (p *Plugin) handleJob(addr string, encoder int, job *order.Job) {
	...
	if job != nil && job.Action == order.KeyDelete {
		...
		if ok := driver.SendKeyDelete(job.Station); !ok {
	...
	go func(addr string, job *order.Job) {
		...
		handleKeyAnswer := func(addr string, in *ifc.LogicalPacket, action *dispatcher.StateAction, job *order.Job) error {
			...
			keyCode := p.getKeyAsString("CardType", in)
			encoder := p.getEncoder(addr)
			encoderAddr := p.getKeyAsInt("EncoderNumber", in)

			if encoder == encoderAddr {
				if in.Name == template.PacketError {
					err := process(addr, in, action, job)
					return err
				} else if job.Action == order.KeyDelete {
					if keyCode == "O" {
						err := process(addr, in, action, job)
						return err
					}
					log.Debug().Msgf("%s addr '%s' mismatching key type '%s' from PMS, '%s' in packet", name, addr, "O", keyCode)
				} else {
					guest, ok := job.Context.(*record.Guest)
					if !ok {
						return errors.Errorf("context '%T' not supported", job.Context)
					}
					keyType := driver.GetKeyType(guest)
					if keyType == keyCode {
						err := process(addr, in, action, job)
						return err
					}
					log.Debug().Msgf("%s addr '%s' mismatching key type '%s' from PMS, '%s' in packet", name, addr, keyType, keyCode)
				}
			} else {
		...
		automate.RegisterRule(template.PacketCodeCardAnswer, waitForAnswer, handleKeyAnswer, dispatcher.StateAction{})
		automate.RegisterRule(template.PacketError, waitForAnswer, handleKeyAnswer, dispatcher.StateAction{})
