...
func (p *Plugin) handleNextAction(addr string, t *ticker.ResetTicker, _ *order.Job) {
	...
	case busy:
		...
		if job != nil && job.Tries >= 3 { // check record error counter
			job.Task = 9999 // mark job as done
			log.Warn().Msgf("%s addr '%s' drop job '%s', err=%s", name, addr, job.Action.String(), "vendor refused the record 3 times")
	...
	case nextAction:
		...
		automate.SetErrorCounter(addr, uint16(job.Tries)) // error counter künstlich erzeugen, handling für ENQ -> ACK, Packet -> NAK, ist bereits durch busy state job.Tries check gedeckelt sein
	...
	case nextRecord:
		...
		if job != nil {
			...
			if job.Task == 0 { // first call, driver pre check
				...
				if err := p.driver.PreCheck(job); err != nil {
