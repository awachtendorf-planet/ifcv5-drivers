...
import (
	...
	"github.com/weareplanet/ifcv5-drivers/plabor/automate/datachange"
	"github.com/weareplanet/ifcv5-drivers/plabor/automate/dbsync"
	"github.com/weareplanet/ifcv5-drivers/plabor/automate/request"
	...
	drv "github.com/weareplanet/ifcv5-drivers/plabor/dispatcher"
...
func registerTemplates(parser *analyser.GenericProtocol) {
	...
	// incoming
	// HERE WE GO
	parser.RegisterIncomingTemplate(template.PacketChargePayTV, &template.TplRecord53Packet{})
	parser.RegisterIncomingTemplate(template.PacketChargeMinibar, &template.TplRecord73PLaborPacket{})
	parser.RegisterIncomingTemplate(template.PacketChargeMinibar, &template.TplRecord73ProDacPacket{})

	parser.RegisterIncomingTemplate(template.PacketReqBill, &template.TplRecord63Packet{})
	parser.RegisterIncomingTemplate(template.PacketMessageRequest, &template.TplRecord54PLaborPacket{})
	parser.RegisterIncomingTemplate(template.PacketMessageRequest, &template.TplRecord54ProDacPacket{})
	parser.RegisterIncomingTemplate(template.PacketMessageConfirmation, &template.TplRecord64ProDacPacket{})
	parser.RegisterIncomingTemplate(template.PacketMessageConfirmation, &template.TplRecord64ProLabPacket{})
	parser.RegisterIncomingTemplate(template.PacketDBSyncRequest, &template.TplRecord91Packet{})

	parser.RegisterIncomingTemplate(template.PacketRoomstatus, &template.TplRecord71Packet{})
	parser.RegisterIncomingTemplate(template.PacketRoomstatus, &template.TplRecord19Packet{})

	// incomin error
	parser.RegisterIncomingTemplate(template.PacketError, &template.TplRecord17PLaborPacket{})
	parser.RegisterIncomingTemplate(template.PacketError, &template.TplRecord17ProDacPacket{})
	parser.RegisterIncomingTemplate(template.PacketError, &template.TplRecord57Packet{})
	...
	parser.RegisterIncomingTemplate(template.PacketEot, &template.TplEOT{})
	...
	// outgoing
	// HERE WE GO
	parser.RegisterOutgoingTemplate(template.PacketCheckIn, &template.TplRecord21Packet{})
	parser.RegisterOutgoingTemplate(template.PacketDataChangeUpdate, &template.TplRecord31Packet{})
	parser.RegisterOutgoingTemplate(template.PacketDataChangeMove, &template.TplRecord22Packet{})
	parser.RegisterOutgoingTemplate(template.PacketCheckOut, &template.TplRecord12Packet{})

	parser.RegisterOutgoingTemplate(template.PacketBillPart, &template.TplRecord23Packet{})
	parser.RegisterOutgoingTemplate(template.PacketBalance, &template.TplRecord33Packet{})

	parser.RegisterOutgoingTemplate(template.PacketMessageSignal, &template.TplRecord14PLaborPacket{})
	// parser.RegisterOutgoingTemplate(template.PacketMessageSignal, &template.TplRecord14ProDacPacket{})
	parser.RegisterOutgoingTemplate(template.PacketMessageDelete, &template.TplRecord44Packet{})

	parser.RegisterOutgoingTemplate(template.PacketMessageBlock, &template.TplRecord24Packet{})
	parser.RegisterOutgoingTemplate(template.PacketMessageEnd, &template.TplRecord34Packet{})

	parser.RegisterOutgoingTemplate(template.PacketWakeupIndirect, &template.TplRecord18Packet{})
	parser.RegisterOutgoingTemplate(template.PacketWakeupDirect, &template.TplRecord08Packet{})
	parser.RegisterOutgoingTemplate(template.PacketWakeupIndirectk, &template.TplRecord18Packetk{})
	parser.RegisterOutgoingTemplate(template.PacketWakeupDirectk, &template.TplRecord08Packetk{})

	parser.RegisterOutgoingTemplate(template.PacketDBSyncFrameStart, &template.TplRecord41FrameStartPacket{})
	parser.RegisterOutgoingTemplate(template.PacketDBSyncFrameStopp, &template.TplRecord41FrameStoppPacket{})
