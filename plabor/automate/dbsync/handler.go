...
func (p *Plugin) handleNextAction(addr string, t *ticker.ResetTicker, Job *order.Job) {
	...
	case resyncStart:
		...
		p.driver.SetSwapState(station, true)
		p.sendPacket(addr, &action, template.PacketDBSyncFrameStart, p.driver.GetTrackingId(addr), nil)
	...
	case resyncEnd:
		p.driver.SetSwapState(station, false)
		p.sendPacket(addr, &action, template.PacketDBSyncFrameStopp, p.driver.GetTrackingId(addr), nil)
		p.sendPacket(addr, &action, template.PacketEot, p.driver.GetTrackingId(addr), nil)
