...
func (p *Plugin) handleJob(addr string, job *order.Job) {
	...
	/*
		// Sharer -> ignore driver packet, send pms ok
	...
	*/
	...
	go func(addr string, job *order.Job) {
		...
		automate.RegisterRule(template.PacketResult, answer, p.handleReply, dispatcher.StateAction{})
		automate.RegisterRule(template.PacketResult, commandSent, p.handleReply, dispatcher.StateAction{})
