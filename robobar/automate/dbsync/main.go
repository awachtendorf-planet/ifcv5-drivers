...
func (p *Plugin) handleRequest(addr string) {
	...
	go func(addr string) {
		...
		if err := do(addr); err != nil {
			...
			// create fake job to finalise linkup state and set robobar to "running" mode
			p.guard.RLock()
			last, exist := p.previousJob[addr]
			p.guard.RUnlock()

			if exist {
				dispatcher := automate.Dispatcher()
				dispatcher.CreateDriverJob(last.Station, last.Action, last.Context, "")
			}

		}

		p.guard.Lock()
		delete(p.previousJob, addr)
		p.guard.Unlock()
