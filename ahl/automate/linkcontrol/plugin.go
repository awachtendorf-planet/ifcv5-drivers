...
import (
	...
	ahl "github.com/weareplanet/ifcv5-drivers/ahl/dispatcher"
	...
	"github.com/weareplanet/ifcv5-drivers/ahl/dispatcher"
...
func New(parent *ahl.Dispatcher) *Plugin {
	return &Plugin{driver: parent}

}

const (
	retryDelay      = ahl.RetryDelay
	packetTimeout   = ahl.PacketTimeout
	nextActionDelay = ahl.NextActionDelay
	aliveTimeout    = ahl.AliveTimeout
	maxError        = ahl.MaxError
	maxNetworkError = 2
...
func (p *Plugin) registerStateMaschine() {
	...
	automate.RegisterRule(template.PacketAck, commandSent, p.handlePacketAcknowledge, dispatcher.StateAction{NextState: linkUp, NextTimeout: nextActionDelay})
	...
	automate.RegisterRule(template.PacketLinkAlive, linkUp, p.handleLinkAlive, dispatcher.StateAction{})
}
