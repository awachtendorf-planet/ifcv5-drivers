...
func (p *Plugin) handlePacket(addr string, _ *ifc.LogicalPacket, action *dispatcher.StateAction, _ *order.Job) error {
...
func (p *Plugin) sendBeaconResponse(addr string, _ *ifc.LogicalPacket, action *dispatcher.StateAction, _ *order.Job) error {
	packet, _ := p.driver.ConstructPacket(addr, template.PacketBeaconResponse, "", nil)
	err := p.automate.SendPacket(addr, packet, action)
	return err
}
