	...
	for {
		...
		case sub := <-subscriber.GetMessages():
			...
			case *syncstate.Event:
				...
				addr := event.Addr
				msgType := event.SyncType

				switch event.State {

				case syncstate.Start:
					p.handleRequest(msgType, addr)
...
func (p *Plugin) handleRequest(msgType int, addr string) {
	...
	go func(addr string) {
		...
		if err := do(addr); err != nil {
			...
			broker := dispatcher.Broker()
			if broker != nil {
				broker.Broadcast(syncstate.NewEvent(addr, msgType, syncstate.Error, err), syncstate.Error.String())
			}
			...
			broker := dispatcher.Broker()
			if broker != nil {
				broker.Broadcast(syncstate.NewEvent(addr, msgType, syncstate.End, nil), syncstate.End.String())
			}
